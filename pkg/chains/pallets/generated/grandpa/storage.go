...
// Make a storage key for State id={{false [114]}}
//
...
// Make a storage key for PendingChange id={{false [115]}}
//
...
func GetNextForced(state state.State, bhash types.Hash) (ret uint32, isSome bool, err error) {
...
func GetNextForcedLatest(state state.State) (ret uint32, isSome bool, err error) {
	key, err := MakeNextForcedStorageKey()
	if err != nil {
		return
	}
...
// Make a storage key for Stalled id={{false [87]}}
//
//	`true` if we are currently stalled.
func MakeStalledStorageKey() (types.StorageKey, error) {
	return types.CreateStorageKey(&types1.Meta, "Grandpa", "Stalled")
}
func GetStalled(state state.State, bhash types.Hash) (ret types1.TupleOfUint32Uint32, isSome bool, err error) {
...
func GetStalledLatest(state state.State) (ret types1.TupleOfUint32Uint32, isSome bool, err error) {
	key, err := MakeStalledStorageKey()
	if err != nil {
		return
	}
...
// Make a storage key for Authorities id={{false [116]}}
//
